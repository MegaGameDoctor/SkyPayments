...
public class MySQLDBManager implements DBManager {
    ...
    public void connect(SkyPayments plugin) throws Exception {
        ...
        this.dataTableName = cfg.getString("database.tablePrefix") + "cart";
        this.itemsTableName = cfg.getString("database.tablePrefix") + "items";

        Class.forName("com.mysql.cj.jdbc.Driver");
        connection = DriverManager.getConnection("jdbc:mysql://" + cfg.getString("database.mysql.host") + "/" + cfg.getString("database.mysql.databaseName") + cfg.getString("database.mysql.arguments"), cfg.getString("database.mysql.user"), cfg.getString("database.mysql.password"));
        connection.prepareStatement("CREATE TABLE IF NOT EXISTS `" + dataTableName + "` ( `orderID` VARCHAR(30) NOT NULL, `serverID` INT NOT NULL, `player` VARCHAR(100) NOT NULL, `type` VARCHAR(30) NOT NULL, `data` TEXT NOT NULL, `status` VARCHAR(30) NOT NULL ) ENGINE = InnoDB;").execute();
        connection.prepareStatement("CREATE TABLE IF NOT EXISTS `" + itemsTableName + "` ( `id` INT NOT NULL AUTO_INCREMENT, `item` LONGTEXT NOT NULL, PRIMARY KEY (`id`)) ENGINE = InnoDB;").execute();
    }
    ...
    public void markOrderAsReceived(String player, String orderID) {
        try {
            PreparedStatement preparedStatement = connection.prepareStatement("UPDATE `" + dataTableName + "` SET status=? WHERE orderID=? AND player=? AND serverID=?");
    ...
    public int saveItemInBase64(String base64) {
        try {
            PreparedStatement preparedStatement = connection.prepareStatement("INSERT INTO `" + itemsTableName + "` (`id`, `item`) VALUES (NULL, ?)", Statement.RETURN_GENERATED_KEYS);
